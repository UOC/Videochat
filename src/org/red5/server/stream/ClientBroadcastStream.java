...
import org.uoc.red5.videoconference.IBroadcastStreamUOC;
...
public class ClientBroadcastStream extends AbstractClientStream implements
		...
		IClientBroadcastStreamStatistics, ClientBroadcastStreamMBean,IBroadcastStreamUOC {
	...
	public void saveAsUOC(String name, User user, Meeting meeting, boolean isAppend) throws IOException,
			ResourceNotFoundException, ResourceExistException {
		//log.debug("SaveAsUOC - name: " + name + " append: " + isAppend);
		// Get stream scope
		...
		// Si FicDemo, Get resource for that filename
		IScope parent=scope;
		...
		if(parent.getName().equals("fitcDemo")){
			...
			if (!res.exists()) {
				...
				/*Actualitza l'XML*/
					...
					vcm.createVideoconferenceXML(user.getVideoName(), user.getFullName(), user.getEmail(), meeting.getTopic(), meeting.getDescription());
			}
			log.debug("Recording file: " + res.getCanonicalPath());
			recordingFile = new FileConsumer(scope, res);
		}
