...
if($loggedin == 1)
{
	...
	if (($rec=$_GET['delete'])&&$delete_from)
	{
		...
		if (file_exists($file = $delete_from . $rec  . ".flv"))
		{
			...
			echo "<BR><BR>";
			echo "This deletes only the video recording. You will have to  manually edit the comment to remove embed code and references. Click on the link below to go to your post and edit the comment!";
			echo "<BR>";
			echo "<p align = 'center'><a href=".home_url().'?p='.$postid.'#comment-'.$commentid." target='_blank'><B> View Comment </B></a></p>";
		}
