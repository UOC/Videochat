...
// Public Methods:
	...
	{
		/*var archivo:String = "Adress.txt";
		var cargador:LoadVars = new LoadVars();
		cargador.onLoad = function(exito:Boolean) {
		if (exito) {
			varConnexion=cargador.var_connexion;
			_root.uri.addItem("hola");
			trace ("Esto es varConnexionDentro: " + varConnexion);
		}
		};
	...
	public function makeConnection(evtObj:Object):Void
	{
		if(uri.length > 0) 
		{
			...
			var goodURI = connection.connect(uri.value, scope, fullName, email);
			if(!goodURI) 
			{
